...
# 2. SIFT를 이용한 특징 검출 (그레이스케일 이미지에서 수행)
...
part_image = cv2.imread('dreamcatcher_part.png')
cv2.imshow('Part Image', part_image)
cv2.waitKey(0)

whole_images = ['dreamcatcher.png', 'heart.jpg', 'BnW.png', 'work.jpg']
...
for image_path in whole_images:
    ...
    if len(good_matches) > 0:
        match_img = cv2.drawMatches(segmented_part, kp1, segmented_whole, kp2, good_matches, None, flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
    ...
    if M is not None:
        ...
        cv2.imshow('Matched Features with RANSAC', result_image)
        cv2.waitKey(0)
        break
